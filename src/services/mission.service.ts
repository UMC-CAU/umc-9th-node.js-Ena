...
import { getRegionExists } from '../repositories/region.repository.js';
import { createMission, getMissionById, getStoreMissions } from '../repositories/mission.repository.js';
import { MissionStoreNotFoundError, MissionRegionNotFoundError, MissionNotFoundAfterCreateError } from '../error.js';
...
export const addMissionService = async (rawBody: any) => {
  ...
  if (!storeOk) {
    throw new MissionStoreNotFoundError('존재하지 않는 가게입니다.', {
      storeId: dto.storeId,
    });
  }

  if (!regionOk) {
    throw new MissionRegionNotFoundError('존재하지 않는 지역입니다.', {
  ...
  if (!mission) {
    throw new MissionNotFoundAfterCreateError(
      '미션 생성 직후 조회에 실패했습니다.',
      { missionId },
    );
  }
